...
export default function BomComparison({ toggleSidebar }: BomComparisonProps) {
    ...
    queryFn: async () => {
      try {
        ...
        const response = await fetch(
          `/api/documents/${selectedDocumentId}/boms`,
        );
        if (!response.ok) {
          console.log(
            `Endpoint /api/documents/${selectedDocumentId}/boms non disponibile, uso logica alternativa`,
          );
          ...
          console.log(
            `Trovate ${sections.length} sezioni nel documento ${selectedDocumentId}:`,
          ...
          console.log(
            `Moduli trovati in tutte le sezioni:`,
            modulesBySection.flat(),
          );
          ...
          const bomModules = modulesBySection
            ...
            .filter((module: any) => {
              console.log(
                `Checking module tipo ${module.type}, content:`,
                module.content,
              );
  ...
  const compareBoms = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/boms/${selectedSourceBomId}/compare/${selectedTargetBomId}`,
      );

      if (!response.ok) {
        // Se l'endpoint non esiste, facciamo un confronto manuale
        const sourceBomResponse = await fetch(
          `/api/boms/${selectedSourceBomId}/items`,
        );
        const targetBomResponse = await fetch(
          `/api/boms/${selectedTargetBomId}/items`,
        );

        if (!sourceBomResponse.ok || !targetBomResponse.ok) {
          throw new Error(
            "Errore nel recupero degli elementi delle distinte base",
          );
        }
        ...
        const sourceItemsByCode = sourceBomItems.reduce(
          (acc: any, item: any) => {
            acc[item.code] = item;
            return acc;
          },
        ...
        const targetItemsByCode = targetBomItems.reduce(
          (acc: any, item: any) => {
            acc[item.code] = item;
            return acc;
          },
        ...
        const commonCodes = Object.keys(targetItemsByCode).filter(
          (code) => sourceItemsByCode[code] !== undefined,
        );

        const uniqueTargetCodes = Object.keys(targetItemsByCode).filter(
          (code) => sourceItemsByCode[code] === undefined,
        );
      ...
      const targetItems = targetBomResponse.ok
        ? await targetBomResponse.json()
        : [];
      ...
      if (data.similarities && data.similarities.length > 0) {
        ...
        console.log(
          `Trovati ${extractedCommonCodes.length} codici comuni dalle similarities`,
        );
      ...
      toast({
        ...
        description:
          "Il confronto tra le distinte base è stato completato con successo",
      });
    } catch (error) {
    ...
    onSuccess: async (data) => {
      try {
        ...
        toast({
          ...
          description:
            "Il nuovo documento è stato creato con successo. Ora puoi accedervi dalla dashboard.",
        });
  ...
  const createDocumentStructure = async (newDocumentId: number) => {
    try {
      ...
      const sections = await sectionsResponse.json();
      console.log(
        `Recuperate ${sections.length} sezioni dal documento originale`,
      );
      ...
      for (const section of sections) {
        const componentsResponse = await fetch(
          `/api/sections/${section.id}/components`,
        );
        if (componentsResponse.ok) {
          ...
          const hasCommonComponents = components.some(
            (comp: any) =>
          ...
          if (hasCommonComponents) {
            ...
            console.log(
              `Sezione ${section.id} (${section.title}) ha componenti comuni`,
            );
          }
        ...
        const modulesResponse = await fetch(
          `/api/sections/${section.id}/modules`,
        );
        if (modulesResponse.ok) {
          ...
          if (hasBomModule) {
            sectionWithComponentMap.set(section.id, true);
            console.log(
              `Sezione ${section.id} (${section.title}) ha moduli BOM rilevanti`,
            );
          }
        }
      }
      ...
      const createSectionHierarchy = async (
        section: any,
        ...
        if (!shouldIncludeSection(section.id)) {
          console.log(
            `Sezione ${section.id} (${section.title}) saltata: nessun componente comune`,
          );
          return;
        }

        console.log(
          `Creando sezione ${section.id} (${section.title}) nel nuovo documento`,
        );
        ...
        const newSectionResponse = await apiRequest(
          "POST",
          "/api/sections",
          newSectionData,
        );
          ...
          try {
            ...
            if (module.type === "bom") {
              if (moduleContent.bomId === parseInt(selectedSourceBomId)) {
                ...
                console.log(
                  `Aggiornato modulo BOM in sezione ${newSection.id}, bomId ${moduleContent.bomId} -> ${selectedTargetBomId}`,
                );
              }
            }
        ...
        for (const comp of components) {
          try {
            if (!comp.component || !comp.component.id) {
              console.error(
                "Componente senza oggetto component o ID valido:",
                comp,
              );
              continue;
            }
            ...
            const isCommonCode =
              componentCode && commonCodes.includes(componentCode);
            ...
            if (isCommonCode) {
              ...
              console.log(
                `Tentativo di associare componente ${componentCode} (ID: ${componentId}) alla sezione ${newSection.id}`,
                newComponentData,
              );
              ...
              if (response.ok) {
                console.log(
                  `SUCCESSO: Copiato componente comune ${componentCode} (ID ${componentId}) alla sezione ${newSection.id}`,
                );
              } else {
                const errorText = await response.text();
                console.error(
                  `Errore nell'assegnazione del componente: ${errorText}`,
                );
              }
            } else {
              console.log(
                `Componente ${componentCode || "senza codice"} non nei codici comuni, non copiato alla nuova sezione`,
              );
            }
          } catch (e) {
      ...
      console.error(
        "Errore nella creazione della struttura del documento:",
        error,
      );
      throw error;
    }
  ...
  const getComparisonSummary = () => {
    console.log(
      "getComparisonSummary chiamata, comparisonResult:",
      comparisonResult,
    );
    ...
    if (comparisonResult.similarities) {
      comparisonResult.similarities.forEach((similarity: any) => {
        if (similarity.similarity === 100) {
          // Solo match perfetti
          const code =
            similarity.item2?.code || similarity.item2?.component?.code;
            ...
            <TabsContent value="compare">
              <Card>
                <CardHeader className="flex justify-between items-start">
                  <div>
                    <CardTitle>
                      Seleziona le Distinte Base da Confrontare
                    </CardTitle>
                    <CardDescription>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="source-bom-select">
                        Distinta Base di Origine
                      </Label>
                      <Select
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="target-bom-select">
                        Nuova Distinta Base
                      </Label>
                      <Select
                  ...
                  <Card>
                    <CardHeader className="flex justify-between items-start">
                      <div>
                        ...
                        <CardDescription>
                          Confronto tra la distinta base originale e la nuova
                          distinta base
                        </CardDescription>
                      </div>
                      <DownloadGuideButton className="ml-4" />
                    ...
                    <CardContent>
                          ...
                          <div className="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div className="p-4 bg-neutral-lightest rounded-lg border border-neutral-light">
                              <p className="text-sm text-neutral-medium">
                                Codici Totali (Target)
                              </p>
                              <p className="text-2xl font-semibold">
                                {summary?.totalTargetCodes}
                              </p>
                            </div>
                            <div className="p-4 bg-neutral-lightest rounded-lg border border-neutral-light">
                              <p className="text-sm text-neutral-medium">
                              ...
                              <p className="text-2xl font-semibold">
                                {summary?.commonCodesCount}
                              </p>
                            </div>
                            <div className="p-4 bg-neutral-lightest rounded-lg border border-neutral-light">
                              <p className="text-sm text-neutral-medium">
                                Codici Non Associati
                              </p>
                              <p className="text-2xl font-semibold text-amber-700">
                                {summary?.uniqueCodesCount}
                              </p>
                            </div>
                            <div className="p-4 bg-neutral-lightest rounded-lg border border-neutral-light">
                              <p className="text-sm text-neutral-medium">
                              ...
                              <div className="flex items-start space-x-2">
                                ...
                                <div>
                                  <h4 className="text-amber-800 font-medium text-sm">
                                    Componenti Non Associati
                                  </h4>
                                  <p className="text-amber-700 text-sm mt-1">
                                    I seguenti {summary?.uniqueCodesCount}{" "}
                                    componenti sono presenti solo nella nuova
                                    distinta base e non hanno associazioni nella
                                    distinta originale:
                                  </p>
                                  <div className="mt-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                                    {summary?.nonAssociatedCodes
                                      ?.slice(0, 9)
                                      .map((item: any, index: number) => (
                                        <div
                                          key={index}
                                          className="px-3 py-2 bg-white border border-amber-200 rounded text-xs"
                                        >
                                    ...
                                    {(summary?.nonAssociatedCodes?.length ||
                                      0) > 9 && (
                      ...
                      <div className="overflow-x-auto">
                        <Table>
                          <TableCaption>
                            Codici non associati nella nuova distinta base
                          </TableCaption>
                          <TableHeader>
                            <TableRow>
                      ...
                      >
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              Inserisci un titolo per il nuovo documento basato
                              sulla nuova distinta base. Verranno copiate tutte
                              le sezioni e i moduli associati ai codici comuni.
                            </DialogDescription>
                          </DialogHeader>
                          ...
                          <div className="space-y-4 py-4">
                            ...
                            <Alert>
                              ...
                              <AlertDescription>
                                Il nuovo documento conterrà tutte le sezioni e i
                                moduli associati ai codici comuni tra le due
                                distinte base.
                              </AlertDescription>
                            </Alert>
                          </div>
