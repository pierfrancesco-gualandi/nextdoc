  ...
  try {
    ...
    if (languageId) {
      ...
      try {
        ...
        // IMPORTANTE: Verifica completezza del documento tradotto
        console.log("📝 Documento tradotto:", {
          titolo: document?.title || 'NON DEFINITO',
          numeroSezioni: documentData.sections?.length || 0,
          primaSezione: documentData.sections[0]?.title || 'NESSUNA SEZIONE'
        });
        ...
        // UTILIZZIAMO SOLO IL CODICE HTML GENERATO MANUALMENTE
        // invece di chiamare exportDocumentHtml che ha molti bug
        console.log("⚠️ Generazione HTML INDIPENDENTE per documento tradotto");
        
        let htmlContent = `
        <!DOCTYPE html>
        <html lang="${languageId === '1' ? 'it' : 'en'}">
        <head>
          <meta charset="UTF-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>${document.title || 'Documento esportato'}</title>
          <style>
            body { font-family: Arial, sans-serif; line-height: 1.6; margin: 0; padding: 20px; }
            .document-header { margin-bottom: 30px; }
            .document-title { font-size: 28px; font-weight: bold; margin-bottom: 10px; }
            .document-meta { color: #666; margin-bottom: 5px; }
            ...
            .module-text { margin-bottom: 15px; }
            .module-image { text-align: center; margin: 15px 0; }
            .module-image img { max-width: 100%; height: auto; }
            .module-caption { font-style: italic; text-align: center; color: #666; }
            .module-warning { background-color: #fff3cd; border-left: 4px solid #ffc107; padding: 15px; }
            .module-danger { background-color: #f8d7da; border-left: 4px solid #dc3545; padding: 15px; }
            .module-note { background-color: #d1ecf1; border-left: 4px solid #17a2b8; padding: 15px; }
        ...
        <body>
          <div class="document-header">
            ...
            <div class="document-meta">Versione: ${document.version || ''}</div>
            <div class="document-meta">${document.description || ''}</div>
          </div>
          <div class="document-content">
        `;
        
        // Funzione ricorsiva per generare HTML delle sezioni
        const generateSectionsHtml = (sections: any[], level = 1) => {
          return sections.map(section => {
            let sectionHtml = `
              <div class="section" id="section-${section.id}">
                <h${level + 1} class="section-title">${section.title || ''}</h${level + 1}>
            ...
            if (section.modules && section.modules.length > 0) {
              ...
              sortedModules.forEach(module => {
                ...
                switch (module.type) {
                  case 'text':
                  case 'testp':
                    sectionHtml += `<div class="module module-text">${content.text || ''}</div>`;
                    break;
                  ...
                  case 'image':
                    sectionHtml += `
                      <div class="module module-image">
                        ...
                        ${content.caption ? `<div class="module-caption">${content.caption}</div>` : ''}
                      </div>
                    `;
                    break;
                  ...
                  case 'warning-alert':
                    sectionHtml += `
                      <div class="module module-warning">
                        <h4>${content.title || 'Avvertenza'}</h4>
                        <p>${content.description || content.message || ''}</p>
                      </div>
                    `;
                    break;
                  ...
                  case 'danger':
                    sectionHtml += `
                      <div class="module module-danger">
                        <h4>${content.title || 'Pericolo'}</h4>
                        <p>${content.description || content.message || ''}</p>
                      </div>
                    `;
                    break;
                  ...
                  case 'note':
                    sectionHtml += `
                      <div class="module module-note">
                        <h4>${content.title || 'Nota'}</h4>
                        <p>${content.description || content.message || ''}</p>
                      </div>
                    `;
                    break;
                  ...
                  case 'bom':
                    // Per la BOM, usa il titolo tradotto
                    sectionHtml += `
                      <div class="module module-bom">
                        <h4>${content.title || 'Elenco componenti'}</h4>
                        <p>${content.description || ''}</p>
                        <!-- La tabella BOM verrà gestita in seguito -->
                      </div>
                    `;
                    break;
                      ...
                      sectionHtml += `
                        <div class="module module-table">
                          <table>
                            <thead>
                              <tr>
                                ${content.headers.map((header: string) => `<th>${header}</th>`).join('')}
                              </tr>
                            </thead>
                            <tbody>
                  ...
                  default:
                    sectionHtml += `<div class="module">Modulo di tipo: ${module.type}</div>`;
                }
              });
            }
        ...
        // Organizza le sezioni in gerarchia per visualizzarle correttamente
        const mainSections = sortedSections.filter(section => !section.parentId);
        const childrenMap: Record<number, any[]> = {};
        
        sortedSections.forEach(section => {
          if (section.parentId) {
            if (!childrenMap[section.parentId]) {
              childrenMap[section.parentId] = [];
            }
            childrenMap[section.parentId].push(section);
          }
        });
        
        // Genera HTML per le sezioni principali
        htmlContent += generateSectionsHtml(mainSections);
        ...
        const fileName = `${document.title || 'documento'}.html`.replace(/\s+/g, '_');
        downloadTextFile(fileName, htmlContent, 'text/html');
        
        console.log("✅ Esportazione HTML completata con successo");
        return;
        
      } catch (error) {
        console.error("❌ Errore durante la generazione HTML manuale:", error);
        // In caso di errore, continua con il metodo standard come fallback
      }
    }
    
    // Se non è stata richiesta una traduzione o se il metodo ottimizzato ha fallito, usa il metodo standard
